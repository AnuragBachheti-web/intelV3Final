...
/* ── Constants ────────────────────────────────────────────────────────────── */
const ALL_CHANNELS = ['Amazon', 'Shopify', 'Walmart Marketplace', 'eBay', 'TikTok Shop'];
...
const compute = (tab, val1, val2, channels) => {
  if (tab === 'inventory') {
...
const IntelSimulationPage = () => {
  ...
  const intelTab         = state?.intelTab || 'sales';

  const cfg          = INPUT_CONFIG[intelTab]    || INPUT_CONFIG.sales;
  const categoryMeta = INTEL_TAB_META[intelTab]  || INTEL_TAB_META.sales;
  const quickPrompts = QUICK_PROMPTS_BY_TAB[intelTab] || QUICK_PROMPTS_BY_TAB.sales;
  const aiSuggestions = AI_SUGGESTIONS_BY_TAB[intelTab] || AI_SUGGESTIONS_BY_TAB.sales;
  ...
  const tableRows = intelTab === 'inventory' ? [
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="number" value={currentPrice} min={1}
                    ...
                    className={`w-full ${cfg.val1Prefix ? 'pl-7' : 'pl-3'} pr-3 py-2 rounded-xl border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-sm font-semibold text-gray-900 dark:text-slate-100 focus:outline-none focus:ring-1 focus:ring-gray-300`} />
                </div>
              </div>
