...
const getStateData = (intelTab) => TAB_STATE_DATA[intelTab] || TAB_STATE_DATA.sales;
...
const IntelV2RollbackPage = () => {
  const location = useLocation();
  const navigate = useNavigate();
  const stateData = location.state || {};

  const insight     = stateData.insight     || null;
  const intelTab    = stateData.intelTab    || 'sales';
  ...
  const { previous, current } = getStateData(intelTab);
...
export default IntelV2RollbackPage;
