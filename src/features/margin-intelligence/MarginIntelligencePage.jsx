          ...
          kpiGroups: [
            {
              ...
              kpis: [
                { label: 'Total Revenue', value: product.revenue || (product.rev ? `$${Number(product.rev).toLocaleString()}` : '—') },
                { label: 'Units Sold', value: product.cogs || '—' },
                { label: 'Avg Price', value: product.cm2 || '—' },
                { label: 'Buy Box %', value: product.gross || (product.bb ? `${(product.bb * 100).toFixed(0)}%` : '—') },
              ],
            },
            {
              label: 'Margin',
              ...
              kpis: [
                { label: 'Margin %', value: product.cm3 || (product.cmpct != null ? `${(product.cmpct * 100).toFixed(1)}%` : '34.2%') },
                { label: 'CM2 Profit', value: product.cm2pct || (product.cm2 != null && typeof product.cm2 === 'number' ? `$${product.cm2}` : '$42.8K') },
                { label: 'Revenue', value: product.revenue || (product.rev ? `$${Number(product.rev).toLocaleString()}` : '—') },
                { label: 'Units', value: product.cogs || '—' },
              ],
            },
            {
              label: 'Inventory',
